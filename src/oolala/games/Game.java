...
public abstract class Game {
  ...
  /**
   * Constructor for Game that initializes: myCreaturesMap is a HashMap that pairs a CreatureIndex
   * with the Creature. myActiveIndices is an ArrayList that stores all the indices of Creatures
   * that are updated/active.
   */
  public Game() {
  ...
  /**
   * Getter method that returns myCreaturesMap, which assigns each Creature with an index starting
   * from 1
   *
   * @return myCreaturesMap
   */
  public Map<Integer, Creature> getCreaturesMap() {
    return myCreaturesMap;
  }
  ...
  /**
   * An abstract method that executes exactly one step of the program and then return to the
   * frontend
   *
   * @param command
   */
  ...
  /**
   * compile will read the entire paragraph the user inputs in the TextBox, then clean the format
   * (remove newlines, comments, extra whitespaces), make each line contain only one step to
   * execute, and check whether all the commands are valid (if not, throw an error so that the
   * frontend and catch it)
   *
  ...
  /**
   * A helper method that breaks one line of commands into lines, each of which contain only one
   * step to execute
   *
   ...
   *
   * @param command is the command in its String format. Need to be converted into a Command
   *                object.
...
//            case -1 -> //tell, indefinite number of arguments
  ...
  /**
   * A static method that checks whether a String is numeric e.g. isNumeric("123") = true but
   * isNumeric("n0ra") = false
   *
   * @param string
   * @return
   */
