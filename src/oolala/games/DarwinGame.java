...
/**
 * DarwinGame class creates a game equipped with components unique to Darwin Simulator application,
 * aka. DarwinGame.
 *
 * @author Norah Tan
 */
public class DarwinGame extends Game {
  ...
  /**
   * Constructor that sets up the Darwin Game and initialized: myInstructionsMap, which assigns an
   * index (starting from 1) to each instruction myIndexMap, which tells us for each Creature (key
   * is the index of that Creature), (value is) the index of the current instruction to be executed
   * next myCommandArgs, which gives the corresponding argument for each instruction
   */
  public DarwinGame() {
    super();
   ...
   * @param radius is the radius determining how far "nearby" is for a creature to sense its
   *               environment
   */
  ...
  /**
   * This method will initialize a Creature with specified location and type to myCreaturesMap in
   * DarwinGame
   *
   ...
   */
  public Map<Integer, Command> getInstructionsMap() {
    return myInstructionsMap;
  }
    ...
    while (true) {
      ...
      if (result.isAction()) {
        switch (result.getNumArgs()) {
          ...
          case 2 -> result.execute(currentCreature,
              this); //infect,other non-action commands except go, ifrandom
        }
