...
/**
 * FractalGame class creates a game equipped with components unique to L-System Visualizer
 * application, aka. FractalGame.
 *
 * @author Norah Tan
 */
public class FractalGame extends Game {
  ...
  /**
   * /** Constructor that sets up the Fractal Game and initialized: myRulesMap, which tells us how
   * each symbol is evolved into the next level
   */
  public FractalGame() {
    super();
    ...
    for (int i = 0; i < myLevels; i++) {
      ...
      for (int j = 0; j < currentSymbols.length(); j++) {
        ...
        if (myRulesMap.containsKey(symbol)) {
          newSymbols =
              newSymbols.substring(0, j) + myRulesMap.get(symbol) + newSymbols.substring(j + 1);
        }
